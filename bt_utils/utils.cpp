...
std::string calculateSHA1(std::string input_message)
{
    ...
    for (int i = 0; i < num_chunks; i++){
        ...
        for (int j = 16; j < 80; j++) {
                ...
                words_big_endian[j] = helperLeftRoate32(temp_xor, 1);
        }

//        printf("MATCHING DEBUG\n");
//        for (int k = 0; k < 80; k++) {
//            printf("%x\n", words_big_endian[k]);
//        }
        ...
        for (int j = 0; j < 80; j++) {
            ...
            uint32_t temp = helperLeftRoate32(a, 5) + f + e + k + words_big_endian[j];
            e = d;
            d = c;
            c = helperLeftRoate32(b, 30);
            b = a;
            a = temp;
...
uint32_t helperLeftRoate32(uint32_t num, int n)
{
